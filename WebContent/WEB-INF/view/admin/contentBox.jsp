	...
	<table id="content_box_table">
		...
		<tr>
			<td colspan="6">
 				전체회원수 : ${count}
 				<br />
			<c:forEach begin="1" end="${Math.ceil(count/5)}" step="1" var="i" ><%--${Math.ceil(list.size()/5)} 스크립틀릿 영역에서 연산하는게 깨끗하지 않다  --%>
				<span>${i}</span>
			</c:forEach>
			</td>
		</tr>
...
//회원수가 수백만명 될 수 있으니까 전체 리스트를 가져올수가없다

/* 전체회원수(query) : ${count}
전체회원수(js) : ${list.size()} 
나누기 : ${Math.ceil(list.size()/5)} */
