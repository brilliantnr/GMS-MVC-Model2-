...
public class SearchCommand extends Command {
	...
	public void excute() {
		...
		String[] arr2 = {request.getServletPath().split("/")[1].split("\\.")[0],
		...
		String searchTeam = request.getParameter("teamname");
		if(searchName!=null) {
			request.setAttribute("column", searchName);
		}else if(searchTeam!=null){
			request.setAttribute("column", searchTeam);
		}
		request.setAttribute("list", MemberServiceImpl.getInstance().search(paramMap));
		request.setAttribute("page", page);
		request.setAttribute("memberCount", MemberServiceImpl.getInstance().count());
		super.excute();
	}
}
